namespace System.Web.Script.Serialization
{
    ...
    public class JavaScriptSerializer
    {
        ...
        internal static object Deserialize(JavaScriptSerializer serializer, string input, Type type, int depthLimit)
        {
            ...
            {
                throw new ArgumentException("Max Json Length Exceeded", "input");
            }
        ...
        public int MaxJsonLength
        {
            ...
            set
            {
                if (value < 1)
                {
                    throw new ArgumentOutOfRangeException("Invalid Max Json Length");
                }
        ...
        public int RecursionLimit
        {
            ...
            set
            {
                if (value < 1)
                {
                    throw new ArgumentOutOfRangeException("Invalid Recursion Limit");
                }
        ...
        internal void Serialize(object obj, StringBuilder output, SerializationFormat serializationFormat)
        {
            ...
            {
                throw new InvalidOperationException("Max Json Length Exceeded");
            }
        }
        ...
        private void SerializeCustomObject(object o, StringBuilder sb, int depth, Hashtable objectsInUse, SerializationFormat serializationFormat)
        {
            ...
            foreach (FieldInfo fieldInfo in fields)
            {
                ...
                sb.Append(':');
                SerializeValue(fieldInfo.GetValue(o), sb, depth, objectsInUse, serializationFormat, currentMember: fieldInfo);
                first = false;
            }
            ...
            foreach (PropertyInfo propInfo in props)
            {
                ...
                sb.Append(':');
                SerializeValue(getMethodInfo.Invoke(o, null), sb, depth, objectsInUse, serializationFormat, currentMember: propInfo);
                first = false;
            }
        ...
        private void SerializeDictionary(IDictionary o, StringBuilder sb, int depth, Hashtable objectsInUse, SerializationFormat serializationFormat)
        {
            ...
            foreach (DictionaryEntry entry in (IDictionary)o)
            {
                ...
                if (key == null)
                {
                    throw new ArgumentException(String.Format(CultureInfo.InvariantCulture, "Dictionary Type Not Supported", o.GetType().FullName));
                }
        ...
        private void SerializeValue(object o, StringBuilder sb, int depth, Hashtable objectsInUse, SerializationFormat serializationFormat, MemberInfo currentMember = null)
        {
            ...
            {
                throw new ArgumentException("Depth Limit Exceeded");
            }
        ...
        private void SerializeValueInternal(object o, StringBuilder sb, int depth, Hashtable objectsInUse, SerializationFormat serializationFormat, MemberInfo currentMember)
        {
            ...
            if (type.IsEnum)
            {
                ...
                if ((underlyingType == typeof(Int64)) || (underlyingType == typeof(UInt64)))
                {
                    ...
                    string errorMessage = (currentMember != null)
                        ? String.Format(CultureInfo.CurrentCulture, "Cannot Serialize Member Generic", currentMember.Name, currentMember.ReflectedType.FullName) + " " + "Invalid Enum Type"
                        : "Invalid Enum Type";
            ...
            try
            {
                ...
                else if (objectsInUse.ContainsKey(o))
                {
                    ...
                    throw new InvalidOperationException(String.Format(CultureInfo.CurrentCulture, "Circular Reference", type.FullName));
                }
