...
namespace System.Web.Script.Serialization
{
    ...
    internal static class ObjectConverter
    {
        ...
        private static bool ConvertDictionaryToObject(IDictionary<string, object> dictionary, Type type, JavaScriptSerializer serializer, bool throwOnError, out object convertedObject)
        {
            ...
            if (IsGenericDictionary(type))
            {
                ...
                if (keyType != typeof(string) && keyType != typeof(object))
                {
                    if (throwOnError)
                    {
                        throw new InvalidOperationException(String.Format(CultureInfo.InvariantCulture, "JSON Dictionary Type Not Supported", type.FullName));
                    }
            ...
            if (type != null && !type.IsAssignableFrom(o.GetType()))
            {
                ...
                {
                    throw new MissingMethodException(String.Format(CultureInfo.InvariantCulture, "JSON No Constructor", type.FullName));
                }

                throw new InvalidOperationException(String.Format(CultureInfo.InvariantCulture, "JSON Deserializer Type Mismatch", type.FullName));
            }
        ...
        private static bool ConvertObjectToTypeMain(object o, Type type, JavaScriptSerializer serializer, bool throwOnError, out object convertedObject)
        {
            ...
            if (o == null)
            {
                ...
                if (IsNonNullableValueType(type))
                {
                    if (throwOnError)
                    {
                        throw new InvalidOperationException("JSON Value Type Cannot Be Null");
                    }
                    else
                    {
                        convertedObject = null;
        ...
        private static bool ConvertObjectToTypeInternal(object o, Type type, JavaScriptSerializer serializer, bool throwOnError, out object convertedObject)
        {
            ...
            if (throwOnError)
            {
                throw new InvalidOperationException(String.Format(CultureInfo.CurrentCulture, "JSON Cannot Convert Object To Type", o.GetType(), type));
            }
            else
            {
                convertedObject = null;
        ...
        private static bool ConvertListToObject(IList list, Type type, JavaScriptSerializer serializer, bool throwOnError, out IList convertedList)
        {
                ...
                if (strongTypedEnumerable.IsAssignableFrom(type))
                {
                    ...
                    else
                    {
                        ...
                        if (t.IsAssignableFrom(type))
                        {
                            if (throwOnError)
                            {
                                throw new InvalidOperationException(String.Format(CultureInfo.InvariantCulture, "Cannot Create List Type", type.FullName));
                            }
                            else
                            {
                                convertedList = null;
            ...
            if (throwOnError)
            {
                throw new InvalidOperationException(String.Format(
                    CultureInfo.CurrentCulture, "Array Type Not Supported", type.FullName));
            }
            else
            {
                convertedList = null;
